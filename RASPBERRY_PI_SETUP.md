...
выполняло **ресэмплинг** через плагин `plug`. Создайте файл `~/.asoundrc`:

```bash
cat > ~/.asoundrc << 'EOF'
# Делаем устройство по умолчанию асимметричным
# (отдельные конфигурации для воспроизведения и записи)
pcm.!default {
    type asym
...
# Воспроизведение: plug + dmix позволяет нескольким приложениям
# писать звук одновременно с автоматическим ресэмплингом
pcm.playback {
    type plug
    slave.pcm "dmix:0,0"
}

# Запись: plug + dsnoop то же самое для входного потока
pcm.capture {
    type plug
...
В файле `0` — это номер карты voiceHAT (видно в выводе `aplay -l`).
Если ваша I²S-карта имеет другой индекс — замените `dmix:0,0` и
`dsnoop:0,0` на правильные.
