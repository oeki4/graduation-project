...
# I²S-DAC (MAX98357A, voiceHAT) не умеет аппаратной громкости —
# чип играет ровно те цифры, что пришли. softvol добавляет
# виртуальный микшер SoftMaster, который реально масштабирует PCM
# перед отправкой на железо.
pcm.softvol {
    type softvol
...
# Воспроизведение через softvol, запись напрямую с карты
pcm.!default {
    type asym
...
> **Почему softvol?** I²S-DAC вроде MAX98357A на voiceHAT не имеет
> аппаратной регулировки громкости — он просто играет цифры, которые
> ему отдали. Поэтому стандартный микшер `Master` физически ни на что
> не влияет: amixer возвращает успех, а звук остаётся прежним. Плагин
> `softvol` добавляет программный регулятор `SoftMaster`, который
> масштабирует PCM-сэмплы перед отправкой на железо — и тогда команды
> «громче», «тише», «громкость 50» работают как ожидается.
