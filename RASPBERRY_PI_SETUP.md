...
## 4.5. Ключ для погоды (OpenWeatherMap)

Навык погоды использует **OpenWeatherMap** как основной источник — он
надёжно работает из России и отдаёт описание погоды на русском. Нужен
бесплатный API-ключ.

1. Зарегистрируйтесь на https://openweathermap.org/api (бесплатно).
2. В личном кабинете во вкладке **API keys** скопируйте ключ.
3. Задайте его через переменную окружения перед запуском:
...
Чтобы ключ задавался автоматически при каждом входе, добавьте строку
`export OWM_API_KEY="ваш_ключ"` в конец файла `~/.bashrc`.

Для systemd-сервиса добавьте в `voice-assistant.service` строку:
```ini
Environment=OWM_API_KEY=ваш_ключ
```

> **Замечание:** если ключ не задан, навык автоматически переключается
> на бесключевой Open-Meteo, а при его недоступности — на wttr.in.
> То есть погода работает и без ключа, но OpenWeatherMap надёжнее.
> Новый ключ активируется на стороне OpenWeatherMap в течение ~10–60
...
│       ├── weather.py            # OpenWeatherMap → Open-Meteo → wttr.in
