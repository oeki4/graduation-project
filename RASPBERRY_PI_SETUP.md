...
```bash
cat > ~/.asoundrc << 'EOF'
# Простейшая конфигурация для I²S-карты на Raspberry Pi.
# plughw = plug + hw — автоматический ресэмплинг без слоя dmix,
# который на медленных CPU даёт underrun.
pcm.!default {
    type asym
...
> буфер маленький и не успевает заполняться → ALSA сыпет
> `snd_pcm_recover: underrun occurred` сотни раз, а звук превращается в
> хрипы. Голосовому ассистенту параллельный доступ не нужен — он
> единственный издатель аудио, поэтому plug+hw достаточно и работает
> заметно стабильнее.

Если вы уже создавали `~/.asoundrc` с dmix по предыдущей версии этой
