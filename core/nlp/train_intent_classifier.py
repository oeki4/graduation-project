...
Поддерживаемые интенты: включить_сказку, узнать_погоду, включить_радио,
                       поставить_таймер, узнать_гороскоп, поставить_напоминание,
                       другой_интент.
"""
import spacy
...
def create_training_data():
    ...
    reminder_examples  = _load_examples("reminder.txt",  "поставить_напоминание")
    ...
    def _cats(active_label: str) -> dict:
        labels = ["включить_сказку", "узнать_погоду", "включить_радио",
                  "поставить_таймер", "узнать_гороскоп",
                  "поставить_напоминание", "другой_интент"]
        return {label: (1.0 if label == active_label else 0.0) for label in labels}

    training_data = []
    for text in fairytale_examples:
        training_data.append({"text": text, "cats": _cats("включить_сказку")})
    ...
    for text in reminder_examples:
        training_data.append({"text": text, "cats": _cats("поставить_напоминание")})
...
def train_model(output_dir="models/intent_model", n_iter=40, patience=10, train_split=0.8):
    ...
    textcat.add_label("поставить_напоминание")
    ...
    print(f"  - Примеров 'поставить_напоминание': {sum(1 for ex in training_data if ex['cats']['поставить_напоминание'] == 1.0)}")
...
def test_model(nlp, test_texts):
    ...
    expected_results = {
        ...
        # Положительные примеры напоминаний
        "напомни через 5 минут полить цветы": "поставить_напоминание",
        "через час напомни выпить таблетку": "поставить_напоминание",
        "напомни в 18:30 позвонить маме": "поставить_напоминание",
        "разбуди меня через 30 минут": "поставить_напоминание",
        "напомни мне через 10 минут проверить почту": "поставить_напоминание",
        "отмени все напоминания": "поставить_напоминание",
        "покажи мои напоминания": "поставить_напоминание",
        "напомни через час позвонить врачу": "поставить_напоминание",
    ...
    for text in test_texts:
        ...
        print(f"   Все scores: сказка={scores['включить_сказку']:.2%}, "
              ...
              f"напоминание={scores['поставить_напоминание']:.2%}, "
...
if __name__ == "__main__":
    ...
    test_texts = [
        ...
        "напомни мне через 10 минут проверить почту",
        "отмени все напоминания",
        "покажи мои напоминания",
        "напомни через час позвонить врачу",
    ]
