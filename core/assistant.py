...
# «стоит» включён намеренно: Vosk часто распознаёт «стой» как «стоит».
_QUICK_STOP_WORDS = (
    "стоп", "стой", "стопп", "стоит",
...
def _contains_stop_word(text: str) -> bool:
    """
    True, если в тексте встречается стоп-слово как отдельное слово,
    БЕЗ ограничения на длину реплики.

    Используется, когда что-то уже играет и пользователь сказал
    «ассистент ... стой» — при этом Vosk мог захватить вместе с командой
    обрывки звучащей сказки/гороскопа из микрофона. В такой длинной
    «грязной» фразе важно лишь наличие стоп-слова — остальное мусор эха.
    """
    text = (text or "").lower()
